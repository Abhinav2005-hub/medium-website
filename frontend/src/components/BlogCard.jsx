import { Link } from 'react-router-dom';
import PropTypes from 'prop-types';

export const BlogCard = ({ id, authorName, title, content, publishedDate }) => {
  return <Link to ={`/blog/${id}`}>
    <div className="p-4 border-b border-slate-200 pb-4 w-screen max-w-md">
      <div className="flex">
        <Avatar name={authorName} />
        <div className="font-extralight pl-2 text-sm flex justify-center flex-col">
          {authorName}
        </div>
        <div className="pl-2 flex justify-center flex-col">
          <Circle />
        </div>
        <div className="pl-2 font-thin text-slate-500 text-sm flex justify-center flex-col">
          {publishedDate}
        </div>
      </div>
      <div className="text-xl font-semibold pt-2">
        {title}
      </div>
      <div className="text-md font-thin">
        {content.slice(0, 100) + "..."}
      </div>
      <div className="text-slate-500 text-sm font-thin pt-4">
        {`${Math.ceil(content.length / 100)} minute(s) read`}
      </div>
    </div>
    </Link>;
};

BlogCard.propTypes = {
  authorName: PropTypes.string.isRequired,
  title: PropTypes.string.isRequired,
  content: PropTypes.string.isRequired,
  publishedDate: PropTypes.string.isRequired,
};

export function Circle() {
  return <div className="h-1 w-1 rounded-full bg-slate-500" />;
}

export function Avatar({ name, size = "small" }) {
    const isSmall = size === "small";
  
    return (
      <div className={`relative inline-flex items-center justify-center overflow-hidden bg-gray-600 rounded-full ${isSmall ? "w-6 h-6" : "w-10 h-10"}`}>
        <span className={`${isSmall ? "text-xs" : "text-md"} font-extralight text-white`}>
          {name[0]}
        </span>
      </div>
    );
  }  

Avatar.propTypes = {
  name: PropTypes.string.isRequired,
  size: PropTypes.number
};

